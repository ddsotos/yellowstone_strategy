...
- [x] モデル保存・再開・評価結果出力を整備する。checkpoint、resume、CSV/JSON出力を扱えるようにする。
- [x] reward を実学習で検証する。学習ログと対戦ログを見て、変な行動を覚える場合は reward を調整する。

- [x] 二値判断モデルへ1枚案・2枚案の即時マイナス増加量を追加し、独立seed対戦で小幅改善を確認する。
- [x] 複数rollout平均targetを実装し、即時マイナス特徴モデルを独立1,000局で再検証する。K=1モデルの改善は再現せず、K=4初回モデルは独立500局でごく小幅な改善だった。
- [x] 2枚heuristic planで2手目全候補への状態遷移を省き、学習botの対戦速度を改善する。
- [x] K=4教師データを9,380件へ増やし、validation early stoppingを導入する。4ターンtarget予測は改善したが、独立500局のloss shareは改善しなかった。
- [x] 半無限評価で1枚→2枚限定・2枚→1枚限定の方向別policyを比較する。200系列では2枚→1枚限定が悪化し、1枚→2枚限定のみ小幅改善候補として残す。
- [x] 1枚→2枚局面だけの専用Advantageモデルと旧モデル合議gateを試す。専用モデル単体は独立300系列で悪化し、合議gateは悪化抑制に有効だが改善幅は小さい。
- [x] 旧K=4モデルの1枚→2枚限定policyで閾値を探索する。新しい900系列では閾値0.5が平均delta -0.00749、95%CI [-0.01358, -0.00139] で暫定本命。
- [x] 手札を配札・補充時に数字昇順へ整列し、1枚案・2枚案で出す数字を観測へ追加する。2枚案の数字は昇順で入力し、観測長は71になった。
- [ ] 1枚→2枚変更局面でfull continuing評価を悪化させるケースを抽出し、補充後手札価値・将来強制失点をtargetまたは特徴量へ追加する。
- [ ] 4ターンtarget上では有利でもfull matchで悪化する変更局面を抽出し、targetへ不足している手札・補充後価値を特定する。
